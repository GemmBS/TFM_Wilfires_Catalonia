...
```{r}
# Creem la classe binària (Severity) seguint Mitsopoulos et al. (2019)
wfc_model_final$severity <- as.factor(ifelse(wfc_model_final$superficie_total_forestal > 10, "High", "Low"))
...
rf_final_model <- train(
  severity ~ tx + ppt + thermal_amplitude + altitude_z + causa + land_cover_id + county_clean + latitude + longitude + month_detected + hour_detected + year + afecto_zonas_interfaz_urbano_forestal,
  data = train_set,
  method = "ranger",
  trControl = fitControl,
  metric = "ROC", # El model triarà els millors paràmetres basant-se en l'AUC
  importance = "permutation"
)
```

```{r}
# Prediccions sobre el test_set (dades mai vistes)
...
# 2. AUC-ROC (per a la teva "discriminative power" del punt 2.6.4)
...
plot(importancia_data, top = 10, main = "Top 10 Drivers of Wildfire Severity", col = "#d95f02")
```
