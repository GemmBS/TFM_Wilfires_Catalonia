...
folds_index <- spatial_folds$folds_list
```
...
fitControl_reg <- trainControl(
  method = "cv",
  index = folds_index,     # IMPORTANT: Això aplica els blocs de 15km
...
```{r}
# 1. Extraure la importància del model de caret (rf_reg_cv)
# Caret guarda la importància dins de 'finalModel'
importance_values <- rf_reg_cv$finalModel$variable.importance

# 2. Crear el dataframe net
importance_df <- data.frame(
  Variable = names(importance_values),
  Importance = importance_values
) %>% arrange(desc(Importance))

# 3. Gràfic (mantenint el teu estil però amb els noms correctes)
library(ggplot2)
...
ggplot(importance_df, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  labs(
    ...
    subtitle = "Regression Model (10-fold Spatial CV)",
    x = "Predictors", 
