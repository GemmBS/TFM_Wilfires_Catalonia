...
fluidRow(
  valueBox(total_fires, subtitle = "Total Fires Recorded", icon = icon("fire", style = "color: #FF4500;"), color = "red"),
  valueBox(round(total_area / 1000, 1), subtitle = "Total Thousand Ha Burnt", icon = icon("tree", style = "color: #2E8B57;"), color = "yellow")
)
```
...
# --- FUNCIÓ D'ESTIL AMB MARGES EXTREMS ---
estil_tfm_horizontal <- function() {
  theme_minimal(base_family = "sans") +
  theme(
    # Títols una mica més petits per evitar que xoquin entre gràfics
    plot.title = element_text(face = "bold", size = 10, color = "#2c3e50", margin = margin(b = 10)),
    axis.text.y = element_text(size = 8.5), 
    axis.text.x = element_text(size = 7.5),
    panel.grid.major.y = element_blank(),
    panel.grid.minor = element_blank(),
    # Marge dret molt gran (40) per encabir els números del geom_text
    plot.margin = margin(10, 40, 10, 5) 
  )
}

# --- APLICACIÓ ALS GRÀFICS AMB EXPANSIÓ DEL 45% ---
...
p1 <- ggplot(wfc_final2, aes(y = reorder(provincia, provincia, function(x) length(x)), x = ..count..)) +
  ...
  # El mult = c(0, 0.45) reserva gairebé la meitat del gràfic per al text a la dreta
...
p3 <- ggplot(area_provincia, aes(y = reorder(provincia, total_area), x = total_area)) +
  ...
  scale_x_continuous(labels = comma, expand = expansion(mult = c(0, 0.45))) +
...
# --- ASSEMBLEA AMB MÉS ESPAI ENTRE COLUMNES ---
final_plot <- (p1 + p2) / (p3 + p4) + 
  plot_layout(widths = c(1, 1)) + # Força columnes iguals
  plot_annotation(
...
p2 <- wfc_final2 %>%
  ...
  # We use hexbin-like logic or very light points to show density without noise
  ...
  labs(
    ...
    subtitle = "Burnt area trend (log scale)",
    x = "Altitude (m)",
    y = "Burnt Area (ha)"
  ) +
  theme_minimal(base_size = 12) +
...
-   **The thermal threshold for severity**: When observing the size of the points (representing **area in ha**), there is a clear shift as we move toward the right of the X-axis. Although fires are frequent between 15°C and 30°C, the **Large Wildfires**---represented by the largest bubbles---are almost exclusively clustered between **25°C and 40°C**. This thermal window represents the "danger zone" where suppression efforts face the greatest challenges due to fuel volatility.
...
-   Martínez Navarro, Ferran, Simón-Soria, Fernando, & López-Abente, Gonzalo. (2004). Valoración del impacto de la ola de calor del verano de 2003 sobre la mortalidad. Gaceta Sanitaria, 18(Supl. 1), 250-258. <http://scielo.isciii.es/scielo.php?script=sci_arttext&pid=S0213-91112004000400040&lng=es&tlng=es>.
