...
land_cover_summary <- wfc_final2 %>%
  group_by(land_cover_desc) %>%
  summarise(
    frequency = n(),
    total_area = sum(superficie_total_forestal, na.rm = TRUE)
  ) %>%
  filter(land_cover_desc != "No data")
...
p5 <- ggplot(top_10_lc_freq, aes(x = reorder(land_cover_desc, -frequency), y = frequency)) +
  # El "tronc" de l'arbre
  geom_segment(aes(xend = reorder(land_cover_desc, -frequency), yend = 0), 
...
p6 <- ggplot(top_10_lc_area, aes(x = reorder(land_cover_desc, -total_area), y = total_area)) +
  # El "tronc" de l'arbre
  geom_segment(aes(xend = reorder(land_cover_desc, -total_area), yend = 0), 
               color = trunk_color, size = 1.5) +
  # La "copa" de l'arbre
  geom_point(size = 8, color = tree_color) +
  geom_text(label = "🌳", size = 4, color = "white", vjust = 0.5) +
