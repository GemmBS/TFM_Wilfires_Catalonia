...
```{r}
# 1. We only focus on the file that failed before
f_missing <- "cobertes-sol-v1r0-2019-2022.tif"
r_path <- here("Data", f_missing)

if (file.exists(r_path)) {
  message("Final step: Extracting data for the 2019-2022 period...")
  
  r_local <- rast(r_path)
  
  # Ensure points match projection
  incendis_proj <- st_transform(incendis_sf, crs(r_local))
  
  # Extract values
  extracted_values <- extract(r_local, vect(incendis_proj))[, 2]
  
  # Assign only to the fires from 2019 to 2022
  idx <- wfc_consolidated$year >= 2019 & wfc_consolidated$year <= 2022
  wfc_consolidated$land_cover_id[idx] <- extracted_values[idx]
  
  # Save the definitive version
  nanoparquet::write_parquet(wfc_consolidated, here("Data", "wfc_consolidated_landcover.parquet"))
  
  print("Final table update:")
  print(table(wfc_consolidated$land_cover_id, useNA = "always"))
} else {
  stop("Wait! The file ", f_missing, " is still not in your Data folder.")
}
```
