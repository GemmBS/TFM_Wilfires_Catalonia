...
```{r data import}
ruta_fitxer <- here("Data", "WildfiresCat1968_2022.xlsx")
...
ruta_fitxer <- here("Data", "WildfiresSpain1968_2023.csv")
...
Cal importar totes les dades al github i fer aproximacions dels tipus de sòl segons els anys d'incendis i els anys disponibles en els tipus de sòl.
```{r}
# 1. Carregar el ràster 
mcsc_2009 <- rast("C:/Users/LENOVO/Downloads/cobertes-sol-v1r0-2009.tif")

# 2. Convertir els teus incendis a format espacial
# Suposem que 'wfc_consolidated' és el teu dataset
incendis_sf <- st_as_sf(wfc_consolidated, coords = c("longitude", "latitude"), crs = 4326)

# 3. Projectar els incendis al sistema del ràster (molt important!)
# Els ràsters de l'ICGC solen estar en UTM 31N (EPSG:25831)
incendis_proj <- st_transform(incendis_sf, crs(mcsc_2009))

# 4. Extreure el valor del píxel per a cada incendi
# Això et donarà un número (el codi de la coberta)
valors_coberta <- extract(mcsc_2009, vect(incendis_proj))

# 5. Afegir-ho al dataset
wfc_consolidated$codi_mcsc <- valors_coberta[, 2]
...
```{r}
# 1. Carregar el ràster d'elevacions (MDT)
mdt <- rast("C:/Users/LENOVO/Downloads/model-elevacions-terreny-topografic-catalunya-5m-2009-2018.tif")

# 2. Calcular el Pendent (Slope)
# El pendent és vital per al Random Forest. El calculem en graus.
pendent <- terrain(mdt, v = "slope", unit = "degrees")

# 3. Visualització ràpida per comprovar que tot és correcte
plot(mdt, main = "Model Digital d'Elevacions")
plot(pendent, main = "Mapa de Pendents (Graus)")
```
